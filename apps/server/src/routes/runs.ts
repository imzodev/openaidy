...
export const runStreamRoutes: FastifyPluginAsync<RunStreamRoutesOptions> = async (app, options) => {
  const { runEvents } = options;

  /**
   * GET /sessions/:sessionId/runs/:runId/stream
   * SSE endpoint for streaming run events
   */
  app.get('/sessions/:sessionId/runs/:runId/stream', async (request: FastifyRequest, reply: FastifyReply) => {
