...
export function createAddonUpdateTool(deps: AddonToolDeps): BuiltinTool {
  return {
    ...
    description: [
      ...
      'WHERE ADDONS LIVE',
      '─────────────────',
      'Addons live in a dedicated directory managed by OpenAidy, SEPARATE from',
      'your agent workspace. Your workspace_write / code_edit / exec_run tools',
      'operate on the workspace and CANNOT patch an addon in a way the addon',
      'loader picks up — files written there are ignored by the addon system.',
      '',
      'ONLY WAY TO UPDATE AN ADDON',
      '───────────────────────────',
      'Use THIS tool (addon_update). It writes the file changes AND keeps the',
      'on-disk addon.json and the DB record in sync. Do NOT use workspace_write,',
      'code_edit, or exec_run to patch addon files — even if a path looks right,',
      'the loader will not see the change.',
      '',
      'WHAT YOU CAN CHANGE',
      '───────────────────',
