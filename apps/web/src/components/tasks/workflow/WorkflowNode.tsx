 ...
 * accented card for 'approval_gate' subtasks (with a pulsing badge
 * while paused). Loop-configured agent subtasks get a small corner
 * badge showing their iteration count.
 */
...
export function WorkflowNode(props: WorkflowNodeProps) {
  ...
  return (
    <div
      ...
      class={`absolute rounded-lg border px-3 py-2 shadow-sm cursor-grab select-none transition-shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${
        STATUS_BG[props.subtask.status] ?? STATUS_BG.pending
      } ${
        props.selected
          ? 'ring-2 ring-primary shadow-md'
          : 'hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600'
      } ${isApprovalGate() ? 'border-l-[3px] border-l-amber-400 dark:border-l-amber-500' : ''}`}
      style={{
    ...
    >
      <div class="flex items-start gap-1.5">
        <span
          class={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold leading-none flex-shrink-0 mt-px ${STATUS_BADGE_BG[props.subtask.status] ?? STATUS_BADGE_BG.pending}`}
        >
      ...
      <Show when={isLoop()}>
        <div
          class="absolute -top-2 -right-2 flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-[10px] font-mono border border-indigo-200 dark:border-indigo-700 shadow-sm"
      ...
      <div
        ...
        class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3.5 h-3.5 rounded-full bg-gray-400 dark:bg-gray-500 border-2 border-white dark:border-gray-900 cursor-crosshair hover:bg-primary hover:scale-125 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
