...
Define a comprehensive testing strategy and verification checklist for the desktop app. Covers integration tests, end-to-end flows, and platform-specific validation.

## Success Criteria

1. All 10 verification checkpoints pass on each platform
2. Manual test checklist documented
3. Automated smoke tests exist for critical paths
4. Performance benchmarks for startup time and memory usage
...
Add to `apps/desktop/` a smoke test suite:

```typescript
// apps/desktop/smoke.test.ts
import { describe, it, expect, beforeAll, afterAll } from 'vitest';

describe('Desktop app smoke tests', () => {
  beforeAll(async () => {
    // Start the app in headless mode for testing
    // This would use Playwright or a custom harness
  });
  ...
  it('service should start and bind to a port', async () => {
  ...
  it('should store and retrieve credentials', async () => {
    await storeCredential('test-key', 'test-value');
    const value = await getCredential('test-key');
    expect(value).toBe('test-value');
    await deleteCredential('test-key');
  });

  it('should render the Solid.js app in WebView', async () => {
