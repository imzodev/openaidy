...
read_when:
  - You want to connect WhatsApp, Telegram, or another messaging platform to OpenAidy
...
Channels let OpenAidy agents receive and reply to real conversations on messaging platforms. Instead of only using the web UI, you can connect a channel so messages from Telegram, WhatsApp, or other platforms create or resume sessions and trigger agent responses.
...
The first supported channel is **WhatsApp**, connected via the Baileys library which links to WhatsApp Web through a QR code scan. No Meta Business account is required.

Future channels (through the same plugin interface):
...
You can run multiple channel instances at the same time. Each instance is independent — it has its own auth state, configuration, and session routing. You could connect two different WhatsApp numbers, or combine WhatsApp and Telegram on the same server.
