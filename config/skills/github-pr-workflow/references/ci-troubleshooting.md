...
#### Fastify Route Scope Mismatch (common cause of 403 on DELETE/PATCH/PUT)

When a server uses per-route permission scopes (e.g. `agents.read`, `agents.write`, `agents.delete`) and a specific HTTP method returns 403 while others work:

**Symptom pattern:** `GET /agents` → 200 but `DELETE /agents/:id` → 403

**Diagnosis:**

```bash
grep -n "requiredScope\|scope" apps/server/src/routes/<route-file>.ts
```

**Common mistakes:**

```typescript
// WRONG: DELETE uses `agents.list` (copy-pasted from GET)
app.delete('/agents/:agentId', {
  preHandler: requireAuth({ authMiddleware, requiredScope: 'agents.list' }),
});

// RIGHT: DELETE must use `agents.delete`
app.delete('/agents/:agentId', {
  preHandler: requireAuth({ authMiddleware, requiredScope: 'agents.delete' }),
});

// Also watch for typos: `agent.delete` (singular) or `agents.wrtie`
```

**Fix:** Each HTTP method must declare its correct scope:
